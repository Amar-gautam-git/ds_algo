...
public class RomanToInteger {
	...
	public static int romanToInt(String s) {
		...
		if (s.length() == 1) {
			return getValueForRomanLetter(chr[0]);
		}
		int sum = 0;
		for (int i = 0; i < chr.length; i++) {
			char c = chr[i];
			int x = getValueForRomanLetter(c);
			int z = 0;
			if (i < chr.length - 1) {
				z = substraction(c, chr[i + 1]);
			}
			if (z != 0) {
				sum = sum + z;
				i = i + 1;
	 ...
	 * integer.
	 */
	private static int substraction(char a, char b) {

		if (getValueForRomanLetter(b) > getValueForRomanLetter(a)) {
			return getValueForRomanLetter(b) - getValueForRomanLetter(a);
		}

		return 0;
	}

	private static int getValueForRomanLetter(char c) {
		switch (c) {
		case 'I':
			return 1;
		case 'V':
